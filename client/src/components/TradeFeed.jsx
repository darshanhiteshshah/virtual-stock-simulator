...
const TradeFeed = () => {
    ...
    return (
        <div className="bg-gray-900 rounded-xl shadow-2xl border border-gray-800 overflow-hidden">
            ...
            <div className="divide-y divide-gray-800 max-h-[500px] overflow-y-auto">
                {trades && trades.length > 0 ? (
                        ...
                        >
                            <div className="flex items-start justify-between">
                                <div className="flex items-start space-x-3 flex-1">
                                    {/* User Avatar */}
                                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-teal-500 to-teal-600 flex items-center justify-center text-white font-bold text-sm shadow-lg flex-shrink-0 ring-2 ring-teal-500/20">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="text-sm text-gray-300 mb-1">
                                            ...
                                            <span className="font-bold text-teal-400">{trade.symbol}</span>
                                            {' @ '}
                                            <span className="font-semibold text-gray-200">₹{parseFloat(trade.price).toFixed(2)}</span>
                                        </div>
                                ...
                                <div className="text-right ml-4 flex-shrink-0">
                                    ...
                                    <div className={`text-xs font-medium flex items-center justify-end space-x-1 ${
                                        trade.type === 'BUY' ? 'text-blue-400' : 'text-green-400'
                                    }`}>
                    ...
                    <div className="p-12 text-center">
                        ...
                        <p className="text-gray-400 font-medium mb-2">No recent trades yet</p>
                        <p className="text-gray-600 text-sm">Be the first to trade!</p>
                    </div>
                )}
            </div>
            ...
            {/* Footer */}
            {trades && trades.length > 0 && (
                <div className="bg-gray-800/50 px-4 py-3 border-t border-gray-800">
                    <div className="flex items-center justify-between text-xs text-gray-400">
                        <span>Showing {trades.length} recent trades</span>
                        <span className="flex items-center space-x-1">
                            <svg className="w-3 h-3 animate-spin text-teal-500" fill="none" viewBox="0 0 24 24">
